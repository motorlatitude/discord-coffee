...
class Dispatcher
  ...
  parseDispatch: (data) ->
    ...
    @discordClient.Logger.debug("[GATEWAYSOCKET] <~ ["+@discordClient.internals.gateway.toUpperCase()+"]: Received "+data.t+" Payload");
    switch data.t
      ...
      when 'RESUMED' then @handleResume(data)
        ...
        @discordClient.Logger.debug("Unhandled Dispatch t: "+data.t, "warn")

  handleReady: (data) ->
    @discordClient.Logger.debug("Gateway Ready, Guilds: 0 Available / "+data.d.guilds.length+" Unavailable", "info")
  ...
  handleChannelCreate: (data) ->
    ...
    if data.d.type == Constants.channelTypes.text
      ...
      @discordClient.Logger.debug("Channel Create Event Occurred with an unknown channel type","warn")
  ...
  handleChannelUpdate: (data) ->
    ...
    if data.d.type == Constants.channelTypes.text
      ...
      @discordClient.Logger.debug("Channel Update Event Occurred with an unknown channel type","warn")
  ...
  handleChannelDelete: (data) ->
    ...
    if data.d.type == Constants.channelTypes.text
      ...
      @discordClient.Logger.debug("Channel Delete Event Occurred with an unknown channel type","warn")
      @discordClient.Logger.debug("Channel has not been removed from discordClient channel object")
  ...
  handleGuildCreate: (data) -> #fired when bot lazy loads available guilds and joins a new guild
    ...
    for i, channel of data.d.channels
      ...
      if channel.type == Constants.channelTypes.voice
        ...
        @discordClient.Logger.debug("Channel Category Registered: "+channel.name)
      else
        @discordClient.Logger.debug("Unknown channel type: "+channel.type,"warn")
    for i, user of data.d.members
      u = user.user
      @discordClient.Logger.debug("Registering User "+u.username+"#"+u.discriminator)
    ...
    @discordClient.Logger.debug("Joined Guild: "+thisServer.name+" ("+thisServer.presences.length+" online / "+(parseInt(thisServer.member_count)-thisServer.presences.length)+" offline)","info")
  ...
  handleMessageReactionAdd: (data) ->
    ...
    if @discordClient.channels[data.d.channel_id]
      ...
      @discordClient.Logger.debug("Message Reaction Add Event Occurred in unknown channel","warn")
  ...
  handleMessageReactionRemove: (data) ->
    ...
    if @discordClient.channels[data.d.channel_id]
      ...
      @discordClient.Logger.debug("Message Reaction Remove Event Occurred in unknown channel","warn")
  ...
  handleMessageUpdate: (data) ->
    ...
    if @discordClient.channels[data.d.channel_id]
      ...
      @discordClient.Logger.debug("Message Update Event Occurred in unknown channel","warn")
  ...
  handleMessageDelete: (data) ->
    ...
    if channel
      ...
      @discordClient.Logger.debug("Message Delete Event Occurred in unknown channel","warn")
  ...
  handleTypingStart: (data) ->
    ...
    if channel
      ...
      @discordClient.Logger.debug("Typing Start Event Occurred in unknown channel","warn")
  ...
  handleUserUpdate: (data) ->
    ...
    if user.id
      ...
      @discordClient.Logger.debug("UserUpdate event occurred for an unknown user")

  handleVoiceStateUpdate: (data) ->
    @discordClient.Logger.debug("VOICE_STATE_UPDATE event caught")
    #console.log data
    if data.d
  ...
  handleVoiceConnection: (data) -> #bot has connected to voice channel
    @discordClient.Logger.debug("Joined Voice Channel","info")
  ...
  handleResume: (data) ->
    @discordClient.Logger.debug("Connection Resumed","info")
